"""
╔══════════════════════════════════════════════════════════════════════╗
║   WP-CVEHunter v3.0 · WordPress Plugin Vulnerability Hunter          ║
    ...
    def analyze_file(self, filepath: Path) -> List[Finding]:
        ...
        for idx, line in enumerate(scope.lines):
            ...
            if "register_rest_route" in line:
                # permission_callback parametresini çevre satırlarda ara
                ctx_str = " ".join(ctx)
                # Callback parametresi eksik mi veya herkese açık mı?
...
class CVEReportGenerator:
    ...
    def _generate_poc_template(self, slug: str, finding: Finding) -> str:
        """Zafiyet tipine göre exploit veya HTTP PoC şablonu oluşturur."""
    ...
    def save_plugin_report(self, plugin: Dict, findings: List[Finding]) -> Optional[Path]:
        ...
        for f in findings:
            ...
            # Her zafiyet için PoC rehberini kaydet
...
class WPCVEHunterOrchestrator:
    def __init__(self, config: Dict):
        ...
        self.session.headers.update({"User-Agent": "WP-CVEHunter/3.0 Research Tool"})
    ...
    def update_dashboard(self, layout: Layout):
        layout["header"].update(Panel(
            f"[bold cyan]WP-CVEHunter v3.0[/bold cyan] | [dim]Taranan: {self.scanned_count} | Es Geçilen: {self.skipped_count} | Zafiyet: [bold red]{self.vuln_count}[/bold red][/dim]",
            border_style="cyan"
        ))
        findings_str = "\n".join(self.recent_findings[-8:]) if self.recent_findings else "[dim]Henüz zafiyet bulunamadı...[/dim]"
        layout["main"].update(Panel(
            f"[yellow]O An Taranan:[/yellow] [bold white]{self.current_plugin}[/bold white]\n\n"
    ...
    def run(self):
        console.clear()
        console.print(Panel(
            "[bold cyan]WP-CVEHunter v3.0 - Tarama Başlatılıyor[/bold cyan]\n"
